...
Ltac nltac_init_nonsubst :=
  try(intuition;
      try solve_false;
      firstorder;
      firstorder).

Ltac nltac_base_nonsubst := 
  try nltac_init_nonsubst;
  try (eauto; eexists; firstorder);
  try (eauto; firstorder; try congruence).

Ltac nltac :=
  try solve
    [nltac_set; nltac_final | repeat nltac_base; abduction; nltac_final].
